...
| `references/` | 仅 skill 核心主题的指引：架构、端点行为、安全等 | 有实现细节时 |
| `scripts/` | 可执行核心代码 | 有逻辑时 |
| `examples/` | 原始输入 → 设计总结 → 示例专属脚本/资源/协议文档（自包含） | 有端到端用例时 |
...
5. 核心主题文档放 `references/`，每个文件只讲一件事；
   示例专属的协议/细节放 `examples/<name>/`，不放进 `references/`；
